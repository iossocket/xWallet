...
- 在 `AppFeature` 中新增 DApp Tab，接入 `DAppConnect` Reducer
...
- **修改文件:** `xWalletApp.swift`（SDK 初始化）, `Features/App/AppFeature.swift`（新增 DApp Tab）, `Features/App/RootView.swift`（Tab 路由）
- **新增 SPM 依赖:** `WalletConnectSwiftV2` (github.com/WalletConnect/WalletConnectSwiftV2)
